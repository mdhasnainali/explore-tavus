...
export async function extractText(path) {
  ...
  // Bengali block is U+0980-U+09FF. Used only to warn when extraction
  // silently mangled the script. Both sides count marks as well as letters,
  // since Bengali vowel signs are \p{M} and would otherwise skew the ratio
  // above 1.
  const bangla = text.match(/[ঀ-৿]/gu)?.length ?? 0;
